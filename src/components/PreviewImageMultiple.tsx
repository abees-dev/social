...
import { Image as ImageType } from 'src/types/Base';
import { Upload } from 'src/types/UploadResponse';
import Image from './Image';

interface PreviewImageProps {
  listImage: Partial<Upload[]>;
}
          ...
          return (
            <Box key={index}>
              {image?.type === 'image*/mp4' ? (
                <CardMediaStyled
                  component="video"
                  src={`${process.env.REACT_APP_UPLOAD_API_URL}${image.url}`}
                  controls
                  loop
                  defaultValue={10}
                />
                ...
                <Image
                  src={isEmpty(image) ? '' : `${process.env.REACT_APP_UPLOAD_API_URL}${image.url}`}
                  sx={{
